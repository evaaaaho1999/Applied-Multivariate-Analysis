...
# --------------------------------------------------------------------------------------------
#dataset
data_train<-read_excel("C:/Users/eva/Desktop/作業 上課資料(清大)/大四下/多變量/final/Data_User_Modeling_Dataset_Hamdi Tolga KAHRAMAN.xls", "Training_Data")
data_train<-as.data.frame(data_train[,1:6])
data_test<-read_excel("C:/Users/eva/Desktop/作業 上課資料(清大)/大四下/多變量/final/Data_User_Modeling_Dataset_Hamdi Tolga KAHRAMAN.xls", "Test_Data")
data_test<-as.data.frame(data_test[,1:6])
head(data_train)
head(data_test)
#---------------------------------------------------------------------------------------------
# PCA
data_pca<-prcomp(data_train[,-6], scale. = T)
summary(data_pca)
...
PCA_tr<-data_pca$x[,1:2]

# --------------------------------------------------------------------------------------------
# EDA
# Scatterplot
qplot(LPR, PEG, colour = UNS, 
      data = data_train)

#Histogram
graph<-gather(data_train[,-6], Attributes, happen_times, 1:5 )
...
table(cut.h.cluster, data_train$UNS)
#----------------------------------------------------------------------------------------
# K-Means
# Normalization
...
# method 3
fviz_gap_stat(clusGap(x = scale(data_train, center = TRUE, scale = TRUE),FUNcluster = kmeans, nstart = 25, K.max = 10, B = 50),maxSE = list(method = "globalmax", SE.factor = 1))
...
# clustering
ggpairs(cbind(data_train, Cluster=as.factor(data_km4$cluster)),
...
# comparison
table(data_km4$cluster, data_train$UNS)
...
#Comparison
table(data_train$UNS, mb$classification)
...
newuser <- as.data.frame(PCA_tr)
newuser$cluster <- results$clustering

A <- xyplot(PC1 ~ PC2, group = cluster, data = newuser, auto.key = 
              list(space = "right"), par.settings = list(superpose.symbol = list(pch = 0, cex 
                                                                                 = 2, col = c("green", "black", "red", "blue"))))
C <- xyplot(results$medoids[,c("PC1")] ~ results$medoids[,c("PC2")], pch = "@", cex 
...
get_clust_tendency(data_train[,-6], n = 50, gradient = gradient_col)
...
rand.index(as.numeric(iris$Species), newuser$cluste)
...
##CLARANS
sil <- silhouette(newuser$cluste, E.dist)
head(sil[, 1:3], 10)
# Silhouette plot
fviz_silhouette(sil)
si.sum <- summary(sil)
si.sum
km_stats <- cluster.stats(E.dist,  newuser$cluste)
km_stats$within.cluster.ss
km_stats$clus.avg.silwidths
