...
# EDA
# Scatterplot
qplot(x, y, data = ruspini)

#Histogram
graph<-gather(ruspini, Attributes, happen_times, 1:2 )
ggplot(graph, aes(x=happen_times, fill=Attributes))+
  ...
  labs(x="happen_times", y="Frequency",
       title="ruspini Attributes - Histograms") +
  theme_bw()

# --------------------------------------------------------------------------------------------
# PCA
data_pca<-prcomp(ruspini, scale. = T)
summary(data_pca)
...
# Distance matrix
E.dist <- dist(ruspini, method="euclidean") # 歐式距離
M.dist <- dist(ruspini, method="manhattan") # 曼哈頓距離
...
# K-Means
# Normalization
dataNorm <- as.data.frame(scale(ruspini))
...
# method 3
fviz_gap_stat(clusGap(x = scale(ruspini, center = TRUE, scale = TRUE),FUNcluster = kmeans, nstart = 25, K.max = 10, B = 50),maxSE = list(method = "globalmax", SE.factor = 1))
...
aggregate(ruspini, by=list(data_km2$cluster), mean)

# clustering
ggpairs(cbind(ruspini, Cluster=as.factor(data_km2$cluster)),
...
p6<-fviz_cluster(data_km6, data = dataNorm)+
  ggtitle("k = 6")
grid.arrange(p2,p3,p4,p6, nrow = 2)
...
#Comparison
table(mb$classification)
plot(mb, what=c("classification"))
# ---------------------------------------------------------------------------------------------

# DBSCAN
library("fpc")
db = fpc::dbscan(ruspini, eps = 0.15, MinPts = 5)
plot(db, ruspini, main = "DBSCAN", frame = FALSE)
library("factoextra")
fviz_cluster(db, ruspini, stand = FALSE, ellipse= T, ellipse.type = "norm", geom = "point")


# -------------------------------------------------------------------------------------------
#Clique
library(meanShiftR)
aa <- data.frame(scale(ruspini$x),scale(ruspini$y))
result <- meanShift(
  as.matrix(aa),                      
...
data(ruspini)
results <- clara(ruspini, 4, metric = "euclidean", samples = 2, pamLike = F)
newruspini <- ruspini
newruspini$cluster <- results$clustering

A <- xyplot(y ~ x, group = cluster, data = newruspini, auto.key = 
...
get_clust_tendency(ruspuni, n = 50, gradient = gradient_col)
...
dunn(E.dist, newruspini$cluster)
...
##CLARANS
sil <- silhouette(newruspini$cluster, E.dist)
head(sil[, 1:3], 10)
# Silhouette plot
fviz_silhouette(sil)
si.sum <- summary(sil)
si.sum
km_stats <- cluster.stats(E.dist,  newruspini$cluster)
km_stats$within.cluster.ss
km_stats$clus.avg.silwidths
