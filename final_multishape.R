...
# EDA
# Scatterplot
qplot(x, y, colour = factor(shape), data = multishapes)

#Histogram
graph<-gather(df, Attributes, happen_times, 1:2 )
ggplot(graph, aes(x=happen_times, fill=Attributes))+
  ...
  labs(x="happen_times", y="Frequency",
       title="multishapes Attributes - Histograms") +
  theme_bw()

# --------------------------------------------------------------------------------------------
# PCA
data_pca<-prcomp(df, scale. = T)
summary(data_pca)
...
# Distance matrix
E.dist <- dist(df, method="euclidean") # 歐式距離
M.dist <- dist(df, method="manhattan") # 曼哈頓距離
...
rect.hclust(tree =hc3, k = 6, border = "green")
rect.hclust(tree =hc3, k = 3, border = "purple")
rect.hclust(tree =hc3, k = 2, border = "pink")
cut.h.cluster <- cutree(tree = hc3, k = 6)
table(cut.h.cluster, multishapes$shape)
#----------------------------------------------------------------------------------------
# K-Means
# Normalization
dataNorm <- as.data.frame(scale(df))
...
# method 3
fviz_gap_stat(clusGap(x = scale(df, center = TRUE, scale = TRUE),FUNcluster = kmeans, nstart = 25, K.max = 10, B = 50),maxSE = list(method = "globalmax", SE.factor = 1))
...
aggregate(df, by=list(data_km2$cluster), mean)

# clustering
ggpairs(cbind(df, Cluster=as.factor(data_km2$cluster)),
...
# comparison
table(data_km2$cluster, multishapes$shape)


data_km6 <- kmeans(dataNorm, centers=6)
p2<-fviz_cluster(data_km2, data = dataNorm)+
  ggtitle("k = 2")
p6<-fviz_cluster(data_km6, data = dataNorm)+
  ggtitle("k = 6")

grid.arrange(p2,p6, nrow = 1)
...
#Comparison
table(multishapes$shape, mb$classification)
...
db = fpc::dbscan(df, eps = 0.15, MinPts = 6)
plot(db, df, main = "DBSCAN", frame = FALSE)
fviz_cluster(db, df, stand = FALSE, frame = FALSE, geom = "point")
...
#Clique
library(meanShiftR)
aa <- data.frame(scale(df$x),scale(df$y))
result <- meanShift(
  as.matrix(aa),                      
...
results <- clara(df, 6, metric = "euclidean", samples = 2, pamLike = F)
newmulti <- df
newmulti$cluster <- results$clustering

A <- xyplot(y ~ x, group = cluster, data = newmulti, auto.key = 
              list(space = "right"), par.settings = list(superpose.symbol = list(pch = 0, cex 
                                                                                 = 2, col = c("green", "black", "red", "blue"))))
C <- xyplot(results$medoids[,c("y")] ~ results$medoids[,c("x")], pch = "@", cex 
...
get_clust_tendency(df, n = 50, gradient = gradient_col)
...
rand.index(as.numeric(iris$Species), newmulti$cluster)
...
##CLARANS
sil <- silhouette(newmulti$cluster, E.dist)
head(sil[, 1:3], 10)
# Silhouette plot
fviz_silhouette(sil)
si.sum <- summary(sil)
si.sum
km_stats <- cluster.stats(E.dist,  newmulti$cluster)
km_stats$within.cluster.ss
km_stats$clus.avg.silwidths
